...
public class MinTransactionBalanceCalculator extends AbstractBalanceCalculator {
	 ...
	 */
	private List<Transaction> getOptimalBalance(List<NetOwnedByUser> creditors, List<NetOwnedByUser> debtors) {
		List<Transaction> shortest = new ArrayList<Transaction>();
		...
		for (int i = 0; i < creditors.size(); ++i) {
			...
			List<Transaction> childrenTransaction = getOptimalBalance(clearedCreditors, clearedDebtors);
			
			// choose the shortest of transactions
			if (i == 0 || childrenTransaction.size() + 1 < shortest.size()) {
				shortest = childrenTransaction;
				shortest.add(new Transaction(debtor.getUser(), creditor.getUser(), exchangedAmount));
			}
		}
	...
	@Override
	protected List<Transaction> calculateBalance() {
